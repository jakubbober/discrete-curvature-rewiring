...
def ricci_cuda(A, A2, d_in, d_out, N, C):
    i, j = cuda.grid(2)
...
def ricci(A, C=None):
    ...
    ricci_cuda[blockspergrid, threadsperblock](A, A2, d_in, d_out, N, C)
    return C
...
def ricci_post_delta(A, x, y, i_neighbors, j_neighbors, D=None):
    N = A.shape[0]
    A2 = torch.matmul(A, A)
    d_in = A[:, x].sum()
    d_out = A[y].sum()
    ...
    ricci_post_delta_cuda[blockspergrid, threadsperblock](A, A2, d_in, d_out, N, D, x, y, np.array(i_neighbors),
