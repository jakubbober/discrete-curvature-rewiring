...
def val(data, model):
    ...
    accs.append(F.nll_loss(model(data.x, data.edge_index)[val_mask], data.y[val_mask]))
    return accs
...
def main(args, d_input, d_output):
    ...
    for i in range(args.num_expriment):
        ...
        model = GCN(d_input, d_output)
        # with open('rewired_Cora.pk', 'rb') as f:
        #     data = pickle.load(f)
        ...
        for epoch in range(0, args.epochs):
            ...
            if val_acc >= best_val_acc or val_loss <= best_val_loss:
                ...
                if wait_step == args.early_stop:
                    ...
                    break
        log = 'Experiment: {:03d}, Test: {:.6f}'
        print(log.format(i + 1, test_acc))
        test_acc_list.append(test_acc * 100)
    log = 'Model_type: {}, Experiments: {:03d}, Mean: {:.6f}, std: {:.6f}\n'
    print(log.format(args.model_type, args.num_expriment, np.mean(test_acc_list),
