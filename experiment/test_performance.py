...
def test_performance(dname: str, curv_type: str, test: bool = True,
    ...
    if not redo_rewiring:
        with open(f'edge_indices/{dname}/edge_index_{curv_type}.pk', 'rb') as f:
    ...
    for seed, state_dict in zip(val_seeds, state_dicts) if not test else zip(test_seeds, state_dicts):
        ...
        if dname in ('Cora', 'Citeseer', 'Pubmed', 'Computers', 'Photo', 'CoauthorCS'):
            # TODO remove duplication with save_models.py
...
if __name__ == '__main__':
    ...
    for name in datasets:
        ...
        for curvature in curvatures:
            print(str(curvature) + ":")
            try:
                out = test_performance(name, curvature, test=True)
                cell = str(round(out[1] * 100, 2)) + " +- " + str(round(out[2] * 100, 2))
            except:
